...
6. **Entity SRP check (v0.1 회고 교훈):**
   - List all entities this plan creates or significantly modifies
   - For each entity, ask: "이 엔티티가 2가지 이상 사용자 시나리오(탐색/저장/추천/상담)에 걸치는가?"
   - If yes → flag for split or log CARL decision with rationale for keeping merged
   - **Why:** v0.1에서 VehicleCard 이중 사용이 Phase 8 리팩토링(43파일, ~45min)을 유발
</step>
...
5. **Data flow direction check (v0.1 회고 교훈):**
   - For each task with data processing logic, verify the flow direction is explicit:
     - Where does the input come from? (사용자 입력 vs API 응답 vs 로컬 DB)
     - Where does the output go? (UI 표시 vs API 전송 vs 로컬 저장)
   - Ambiguous flow → add explicit direction in task `<action>`: "사용자 입력 → X → API"
   - **Why:** v0.1에서 _extractQuery가 사용자 입력 대신 AI 응답에서 추출하여 버그 발생
