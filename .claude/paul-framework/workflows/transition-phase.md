...
<step name="domain_model_health_check">
**Domain model drift detection — runs at phase transition (v0.1 회고 교훈)**

Check if domain model confusion has accumulated during this phase:

1. **Entity naming audit:**
   - List entities created/modified in this phase's SUMMARY files
   - For each entity: does the name clearly express ONE business concept?
   - Red flag: entity used in 2+ unrelated features with different semantics
   - Example: `VehicleCard` used as both "차량 스펙" and "AI 상담 결과"

2. **Repository responsibility audit:**
   - Does any repository handle 2+ distinct domains?
   - Red flag: `ICardRepository` with methods for vehicle lookup AND bookmark AND garage
   - Each repository should map to ONE aggregate root

3. **If confusion detected (2+ Phase 누적):**
   ```
   ════════════════════════════════════════
   ⚠️ DOMAIN MODEL DRIFT DETECTED
   ════════════════════════════════════════

   Entity/Repository with multiple responsibilities:
   - {entity}: used for {scenario1} AND {scenario2}

   Recommendation: Insert a Domain Separation phase
   before proceeding to Phase {N+1}.

   This adds short-term cost (~1-3 plans) but prevents
   confusion from compounding in every subsequent plan.

   [1] Insert separation phase | [2] Acknowledge and proceed
   ════════════════════════════════════════
   ```

4. **If clean:** Silent pass. Zero friction.

**Why this exists:** v0.1 Phase 5-7에서 VehicleCard 네이밍 혼동과 북마크=차고 혼동이
누적되어 Phase 8 삽입(43파일, ~45min)이 필요했음. 조기 감지했다면 비용이 더 낮았을 것.
</step>
