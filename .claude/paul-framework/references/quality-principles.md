...
**Entity names must express business intent, not UI structure. (v0.1 회고 교훈)**

Claude infers business meaning from class/variable names. If the name is ambiguous, Claude will default to UI interpretation and conflate unrelated domains.

**Single Responsibility for Entities:**
- Each entity should serve ONE user scenario (탐색 OR 저장 OR 추천 OR 상담)
- If an entity spans 2+ scenarios → split it or explicitly document why it stays merged

**Naming anti-patterns:**
| Bad | Why | Good |
|-----|-----|------|
| `VehicleCard` | UI인지 데이터인지 모호 | `Vehicle` (스펙) + `ConsultationCard` (상담 결과) |
| `saveToGarage` for bookmarks | 찜(북마크)과 차고 저장의 의도 차이 무시 | `toggleBookmark()` vs `saveToGarage()` |
| `extractQuery(aiResponse)` | 데이터 흐름 방향 혼동 | `extractQueryFromUserInput(message)` |

**Domain drift signals (catch early):**
- Same entity used in 2+ unrelated features with different field subsets
- Repository with methods spanning multiple aggregate roots
- Function reading from wrong data source (input vs output confusion)

**When drift accumulates over 2+ phases:**
Insert a Domain Separation phase before adding more features.
Short-term cost (~1-3 plans) is lower than compounding confusion in every subsequent plan.
