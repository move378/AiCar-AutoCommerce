...
| **시승찾기** | WebView | 외부 브랜드 전시장 페이지로 리다이렉션 |
| **챗봇** | 네이티브 | AI 상담 → 카드형 차량 추천 |
| **차고** | 네이티브 | 저장된 카드 목록 + 상담 기록. 헤더 톱니바퀴 → 마이페이지 |

> **마이페이지**: GNB 탭이 아닌 차고 헤더 톱니바퀴에서 push 진입. 로그인 필요.

### 4-2. 핵심 플로우

```
Splash (앱 권한 획득)
  → 차량조회 온보딩 (차량번호→소유자명→공공API→등록, "아직 차량이 없어요" 스킵 가능)
  → GNB 홈
       │
       ├── [챗봇 탭] AI 상담
       ...
       │
       ├── [차고 탭] 저장된 카드 목록 (로그인 필요)
       │     ├── 미로그인 → 로그인 유도 → SNS 로그인 → 약관 동의 → 차고 복귀
