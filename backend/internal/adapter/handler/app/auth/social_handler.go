...
// @Produce     json
// @Security    BearerAuth
// @Param       request body dto.AppleLoginRequest true "애플 로그인 요청"
...
func (h *AuthHandler) AppleLogin(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.SendError(c, http.StatusBadRequest, response.CodeBadRequest, "애플 ID 토큰이 필요합니다.")
		return
	}

	result, err := h.appleUsecase.AppleLogin(c.Request.Context(), userUUID, req.IdentityToken, &req.UserName)
	...
	if err != nil {
		response.SendError(c, http.StatusInternalServerError, response.CodeInternalError, "애플 로그인 처리 중 오류가 발생했습니다.")
		return
	}
